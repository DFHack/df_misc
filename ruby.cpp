...
static VALUE rb_m_T_cursor_x(VALUE self) {
    struct T_cursor *var;
    Data_Get_Struct(self, struct T_cursor, var);
...
static VALUE rb_m_unit_id(VALUE self) {
    struct unit *var;
    Data_Get_Struct(self, struct unit, var);
...
static VALUE rb_m_world_T_units_all(VALUE self) {
    struct world::T_units *var;
    Data_Get_Struct(self, struct world::T_units, var);

    // read a vector
    VALUE ret = rb_ary_new();
    for (unsigned i=0 ; i<v->size() ; ++i)
        rb_ary_push(ret, Data_Wrap_Struct(rb_c_unit, 0, 0, v[i]));
...
static VALUE rb_m_world_units(VALUE self) {
    struct world *var;
    Data_Get_Struct(self, struct world, var);
